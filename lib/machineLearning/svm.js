...
function train(svm, namespace, classification, features, callback) {
    ...
    var featuresRow = featureVals; //featuresRowFromFeaturesObject(namespaceDict, featureVals);
    ...
    util.log.debug({
        Features: {vals: featureVals, row: featuresRow},
        Class: {name: classification, id: classId},
        Dataset: dataset
    });
    try {
...
function featuresRowFromFeaturesObject(namespaceDict, featureVals) {
    return _.reduce(namespaceDict.columns, function (row, featureKey) {
        var val = featureVals[featureKey];
